...
- MinIO/Dockerfile hardening (2026-08-31): Fixed MinIO container startup failures caused by CRLF line endings in `compose.yaml` on Windows. Pinned MinIO server to `RELEASE.2025-09-07T16-13-09Z` (was `latest@sha256`). Added `restart: unless-stopped` to MinIO for crash resilience. Pinned mc init container to `latest@sha256:a7fe349...` (different release cycle than server). Changed mc bucket policy from `anonymous set none` to `anonymous set private`. Added `restart: "no"` to mc to prevent init loops. Added `*.yaml`/`*.yml` to `.gitattributes` for LF normalization. Restructured Dockerfile: `deps` stage now copies ALL source code (`packages/` and `apps/`) before `pnpm install`, eliminating the problem where `COPY apps/web ./apps/web` in the `build-web` stage destroyed pnpm workspace symlinks that `pnpm install` had created. Build stages (`build-api`, `build-worker`, `build-web`) no longer COPY source code or re-run `pnpm install`. Added build-time placeholder env vars (`AUTH_DATABASE_URL`, `BETTER_AUTH_SECRET`, etc.) to `build-web` stage so Next.js can validate auth config during `next build`. Set `WORKDIR /app/apps/web` in the web target and fixed CMD from `.bin/next` shell shim to `next/dist/bin/next` JS entrypoint. All 10 services verified running and healthy.
